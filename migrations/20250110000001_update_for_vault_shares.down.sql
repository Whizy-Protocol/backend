-- ================================================
-- Rollback: Remove vault-based contract changes
-- ================================================

-- Remove new columns from markets_extended
ALTER TABLE markets_extended 
DROP COLUMN IF EXISTS "vaultAddress",
DROP COLUMN IF EXISTS "totalYesShares",
DROP COLUMN IF EXISTS "totalNoShares",
DROP COLUMN IF EXISTS "yieldWithdrawn";

-- Remove shares column from bet_placeds
ALTER TABLE bet_placeds
DROP COLUMN IF EXISTS shares;

-- Make blockchainBetId NOT NULL again
ALTER TABLE bets_extended 
ALTER COLUMN "blockchainBetId" SET NOT NULL;

-- Remove shares column from bets_extended
ALTER TABLE bets_extended
DROP COLUMN IF EXISTS shares;

-- Remove vault_address from market_createds
